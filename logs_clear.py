...
parser.add_argument('-t','--timecut', metavar='HOURS', type=float, help='Time value, in hours before current time. This value will determine the cutoff--i.e. you will not delete .LOG files created after that time.')
parser.add_argument('-s','--season', metavar='SEASON', type=int, help='season number')
parser.add_argument('-d','--delete', metavar='True/False', help='If False, code will simply find .LOG files satisfying criteria. If True, they will be deleted as well.\n(default=False)',default=False)
parser.add_argument('-l','--listfile', metavar='LISTFILE', type=str, help='Filename to output list of .LOG files to',default = 'logs.list')
    ...
    for c in chips:
        ...
        if os.path.isdir(cpath):
            for f in os.listdir(cpath):
                ...
                #h, m = divmod(m, 60)
                if delta.total_seconds() > (timeagocut_s) and '.LOG' in f:
                    #print "%d:%02d:%02d" % (h, m, s)
                    #print fullpath
                    alllog.append(fullpath)
                    lf.write(fullpath+'\n')
                    if delete:
                        os.remove(fullpath)
    lf.flush()
...
lf.close()
#np.savetxt(listfile,alllog,fmt='%s')
