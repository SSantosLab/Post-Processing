...
def ZapHTML(Dict,OMDict,theDat,datInfo,LightCurveName): #Dict with obs and associated gifs, dict with OBJIDS and associated MJDS,list of tar files that correspond to observations, list=[snid,raval,decval]
    Name='theProtoATC'+theDat+'.html'
    htmlYeah=open(Name,'w+')
    topLines=['<!DOCTYPE HTML>\n','<html>\n','<head>','<link rel="stylesheet" type="text/css" href="theProtoAtCStyleSheet.css">','<title> Plots from '+theDat+'</title>\n','<h1>This is the title for '+theDat+'</h1>','\n','</head>\n','<body>','<p> This is what it is about. </p>','<table style="width:100%">','<tr>','<th>SNID</th>','<td>'+str(datInfo[0])+'<td>','<th>RA</th>','<td>'+str(datInfo[1])+'<td>','<th>DEC</th>','<td>'+str(datInfo[2])+'</td>','<th>HOST_ID</th>','<td>'+str(datInfo[3])+'</td>','<th>PHOTO_Z</th>','<td>'+str(datInfo[4])+'</td>','</tr>','<tr>','<th>PHOTO_ZERR</th>','<td>'+str(datInfo[5])+'</td>','<th>'+str(datInfo[5])+'</th>',' <td>'+str(datInfo[6])+'</td>','<th>SPEC_ZERR</th>','<td>'+str(datInfo[7])'</td>','<th>HOST_SEP</th>','<td>'+str(datInfo[8])+'</td>','</tr>','<tr>','<th>H_GMAG</th>','<td>'+str(datInfo[9])+'</td>','<th>H_RMAG</th>',' <td>'+str(datInfo[10])+'</td>','<th>H_IMAG</th>','<td>'+str(datInfo[11])+'</td>','<th>H_ZMAG</th>','<td>'+str(datInfo[12])+'</td>','</tr>']
    for tag in topLines:
        htmlYeah.write(tag)
    htmlYeah.close()
