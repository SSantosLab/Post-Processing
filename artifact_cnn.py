...
class StampEvaluator:
    ...
    #Removed colons #Testing for Python 2
    def __init__(
            self, stamps, psf_array = None,
            flux_array = None, fluxerr_array = None,
            masking_box_width = 14, model_dir = '.'):
    ...
    #Changed some f-Strings to Python 2 #Testing for Python 2
    def load_cnns(self):
        """Instantiates CNNs and stores them as class attributes."""
        self.cnn1 = cnn_utils.CNN()
        self.cnn1.load_state_dict(torch.load('{}/model1.pt'.format(self.model_dir)))
        #self.cnn1.load_state_dict(torch.load(f'{self.model_dir}/model1.pt'))
        self.cnn1.eval()
        self.cnn2 = cnn_utils.CNN()
        self.cnn2.load_state_dict(torch.load('{}/model2.pt'.format(self.model_dir)))
        #self.cnn2.load_state_dict(torch.load(f'{self.model_dir}/model2.pt'))
        self.cnn2.eval()

    #Commented out all instances of the arrow (->) #Testing for Python 2
    ...
    @staticmethod
    def _scale_scores(
            scores, cnn_attribute_name): #str) #-> np.ndarray:
        """
